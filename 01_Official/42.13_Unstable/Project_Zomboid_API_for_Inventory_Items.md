...
[Official] [Inventory] [AntiCheat] [MP]
...
[Official] [TimedAction] [Migration] [BestPractice]
...
_Code piece 1 - Example of the incorrect `new` function_
...
_Code piece 2 - realisation of the ISBaseTimedAction:new function_
...
_Code piece 3 - An example of the correct `new` function implementation_
...
_Code piece 4 - the usage of the getDuration function in the `new` function to receive the execution time of a Timed Action_
...
_Code piece 5 - Typical realisation of the getDuration function, returning the 1 second time_

### 2.3. Split of the `perform` function into `perform` and `complete`

[Official] [TimedAction] [Migration]
...
_Code piece 6 - The obsolete realisation of the `perform` function for the ISAddFuelAction timed action_
...
_Code piece 7 - The new realisation of the `perform` function of the ISAddFuelAction timed action_
...
_Code piece 8 - Implementation of the `addFuel` command handler_
...
_Code piece 9 - Sending of the addFuel command from client to server_
...
_Code piece 10 - Search and modifying of the object_
...
_Code piece 11 - Realisation of the `campfireAt` function_
...
_Code piece 12 - The new realisation of the `complete` function of the ISAddFuelAction timed action_

## 3. Realisation features of the long Timed Actions

[Official] [TimedAction] [Example]
...
_Code piece 13 - Example of realisation of the serverStart function_
...
_Code piece 14 - Example of realisation of the `animEvent` function_
...
_Code piece 15 - Realisation of the end of the Timed Action_

## 4. The use of the `sendClientCommand`

[Official] [Command] [MP] [Example]
...
_Code piece 16 - Sending the `getKey` command for the `vehicle` module with an argument `vehicle = self.vehicle:getId()`_
...
_Code piece 17 - Example of implementation of the `getKey` command handler on the server side_
...
This function works only if the player has the UseMechanicsCheat permission. In case if the player has the appropriate permission and the required vehicle is found on the server side then it creates the key InventoryItem on the server side using the "createVehicleKey" function, and after that adds it to the player's inventory and sends it back to the client side.

## 5. Synchronisation of the creation, deletion and modification of items and objects

[Official] [Sync] [Inventory] [MP]
...
_Code piece 18 - Adding InventoryItem to the inventory_
...
_Code piece 19 - Deleting the InventoryItem from the inventory_

Here are some examples of the code for how to create, delete and manipulate objects.

```lua
local trap = IsoTrap.new(self.weapon, self.square)
self.square:AddTileObject(trap)
trap:transmitCompleteItemToClients()
```

_Code piece 20 - Adding the IsoObject to the map_
...
_Code piece 21 - Deleting the IsoObject from the map_
...
_Code piece 22 - IsoObject synchronisation on the map_
